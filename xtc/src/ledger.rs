...
impl Ledger {
    ...
    pub fn balance(&self, account: &Principal) -> u64 {
        *(self.balances.get(account).unwrap_or(&DEFAULT_BALANCE))
    }

    #[inline]
    pub fn deposit(&mut self, account: &Principal, amount: u64) {
        StatsData::deposit(amount);
        *(self.balances.entry(*account).or_insert(DEFAULT_BALANCE)) += amount;
    }
    ...
    pub fn withdraw(&mut self, account: &Principal, amount: u64) -> Result<(), ()> {
        ...
        let balance = self.balances.entry(*account).or_insert(DEFAULT_BALANCE);
        if *balance >= amount {
            *balance -= amount;
        } else {
            return Err(());
...
mod tests {
    ...
    fn approve_and_transfer() {
        ...
        ledger.approve(&alice(), &bob(), 1000, 0);
        assert_eq!(ledger.balance(&alice()), DEFAULT_BALANCE + 500);
        assert_eq!(ledger.balance(&bob()), DEFAULT_BALANCE);
        ...
        // balances changed
        assert_eq!(ledger.balance(&alice()), DEFAULT_BALANCE + 100);
        assert_eq!(ledger.balance(&bob()), DEFAULT_BALANCE);
        assert_eq!(ledger.balance(&charlie()), DEFAULT_BALANCE + 400);
        ...
        assert_eq!(ledger.balance(&alice()), DEFAULT_BALANCE + 1000);
        assert_eq!(ledger.balance(&bob()), DEFAULT_BALANCE);
        assert_eq!(ledger.balance(&charlie()), DEFAULT_BALANCE);
        assert_eq!(
            ledger
                .transfer_from(&bob(), &alice(), &charlie(), 600, 0)
                .unwrap_err(),
    ...
    fn balance() {
        ...
        let mut ledger = Ledger::default();
        assert_eq!(ledger.balance(&alice()), DEFAULT_BALANCE);
        assert_eq!(ledger.balance(&bob()), DEFAULT_BALANCE);
        assert_eq!(ledger.balance(&charlie()), DEFAULT_BALANCE);

        // Deposit should work.
        ledger.deposit(&alice(), 1000);
        assert_eq!(ledger.balance(&alice()), 1000 + DEFAULT_BALANCE);


        assert!(ledger.withdraw(&alice(), 100).is_ok());
        assert_eq!(ledger.balance(&alice()), DEFAULT_BALANCE + 900);
        assert!(ledger.withdraw(&alice(), DEFAULT_BALANCE + 1000).is_err());

        assert!(ledger.withdraw(&alice(), DEFAULT_BALANCE + 900).is_ok());
